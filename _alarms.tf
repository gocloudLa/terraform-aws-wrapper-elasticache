...
locals {
  ...
  alarms_default_tmp = merge([
    for elasticache_name, values in try(var.elasticache_parameters, []) : {
      ...
      merge(
        ...
        {
          alarm_name          = alarm
          alarm_description   = try(values.alarms_overrides[alarm].description, "Elasticache[${elasticache_name}] ${value.description}")
  ...
  alarms_custom_tmp = merge([
    for elasticache_name, values in try(var.elasticache_parameters, []) : {
      ...
      ) if can(var.elasticache_parameters) && var.elasticache_parameters != {} && try(values.enable_alarms, var.elasticache_defaults.enable_alarms, false)
    }
  ]...)
  ...
  alarms_for_node = merge(flatten([
    ...
    ] if can(var.elasticache_parameters) && var.elasticache_parameters != {} && try(elasticache_config.enable_alarms, var.elasticache_defaults.enable_alarms, false)
  ])...)
